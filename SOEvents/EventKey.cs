...
namespace Heimdallr.Core
{
    ...
    [CreateAssetMenu(fileName = "AresEvent_", menuName = "Keys/AresEvent Key")]
    public class EventKey : GenericKey
    {
        [SerializeField] private bool _isAsyncEvent;
        public bool IsAsyncEvent => _isAsyncEvent;

        [FormerlySerializedAs("_onlyGlobal")] [OnValueChanged("OnOnlyGlobalChanged")] [SerializeField]
        private bool _mustBeGlobal;

        public bool MustBeGlobal => _mustBeGlobal;

        [SerializeField]
        [OnValueChanged("OnArg1Changed")]
        [HideIf("IsAsyncEvent")]
        [InlineButton("SelectArg1Type", " ")]
        private string _arg1Type;
        ...
        [SerializeField]
        [OnValueChanged("OnArg2Changed")]
        [HideIf("IsAsyncEvent")]
        [InlineButton("SelectArg2Type", " ")]
        private string _arg2Type;
        ...
        [SerializeField]
        [OnValueChanged("OnReturnChanged")]
        [HideIf("IsAsyncEvent")]
        [InlineButton("SelectReturnType", " ")]
        private string _returnType;

        public string ReturnType => _returnType;
        
#if UNITY_EDITOR
        [ShowInInspector][HideInEditorMode]public List<object> Listeners = new List<object>();
#endif


        #region Methods

        public void SetupKey(string id, bool mustBeGlobal = false, string arg1Type = "", string arg2Type = "",
            string returnType = "", bool isAsyncEvent = false)
        {
            ID = id;
        ...
        private void ResetNameEditor()
        {
            ...
            if (!IsAsyncEvent)
            {
                starting = (string.IsNullOrEmpty(_returnType) ? "ES_" : "AER_");
            }
            else
            {
                starting = "AE_Async_";
            ...
            string newName = starting + globalSymbol + _arg1Type + firstArgSeparator + _arg2Type + secondArgSeparator +
                             _returnType + returnArgSeparator + ID;
            name = newName;
