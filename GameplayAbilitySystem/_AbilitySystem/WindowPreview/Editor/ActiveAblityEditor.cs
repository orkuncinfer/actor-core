...
public class ActiveAblityEditor : OdinEditor
{
            ...
            if(_abilityDefinition.AbilityActions.Count > 0)
            {
                foreach (var action in _abilityDefinition.AbilityActions)
                {
                    ...
                    if (_cachedAnimWindows.ContainsKey(action))
                    {
                        ...
                        {
                            SetPreviewByNormalized(animWindow.x);
                        }else if(!Mathf.Approximately(animWindow.y, _cachedAnimWindows[action].y))
                        {
                            SetPreviewByNormalized(animWindow.y);
                        }
                    }
    ...
    private void SetPreviewByNormalized(float normalized)
    {
        if (_abilityDefinition?.AnimationClip == null || _previewAnimator == null)
            return;

        normalized = Mathf.Repeat(normalized, 1f); // or Clamp01 if you never want multi-loop offsets
        _isPlaying = false;
        _cachedNormalizedTime = normalized;
        _previewTime = normalized * _abilityDefinition.AnimationClip.length;

        _previewAnimator.Play(_abilityDefinition.AnimationClip.name, -1, normalized);
        _previewAnimator.Update(0);
    }
            ...
            if (!hasClip)
            {
                _previewAnimator.runtimeAnimatorController =
                    AnimatorController.CreateAnimatorControllerAtPathWithClip(
                        "Assets/Plugins/actor-core/GameplayAbilitySystem/_AbilitySystem/WindowPreview/PreviewController.controller", clip);
            }
