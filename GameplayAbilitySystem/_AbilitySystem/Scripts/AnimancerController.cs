...
public class AnimancerController : MonoBehaviour
{
    ...
    private void ProcessAbilities()
    {
        ...
        for (int i = _abilityController.GetActiveAbilities().Count ; i > 0; i--) // ANIMATION END CHECK
        {
            ...
            {
                float animDuration = ability.AnimancerState.Clip.length / ability.AnimancerState.Speed;
                ability.AnimNormalizedTime = (Time.time - ability.AbilityStartTime) / animDuration;
                if (ability.AnimNormalizedTime >= ability.Definition.EndTime) // animation casually finished
                {
                    _isLooping = false;
                    Debug.Log("animation finished " + ability.AnimNormalizedTime + "-" + ability.Definition.EndTime);
                ...
                if (ability.AnimancerState.EffectiveWeight <= 0 && ability.AnimancerState.EffectiveWeight < ability.PreviousAnimWeight) // ANIMATION INTERRUPT CHECK
                {
            ...
            foreach (var action in _actionsToProcess)
            {
                ...
                if (action.AnimWindow.x <= action.ActiveAbility.AnimNormalizedTime)
                {
                ...
                if (action.AnimWindow.y <= action.ActiveAbility.AnimNormalizedTime && action.IsRunning)
                {
                    action.OnExit();
                }
                if (action.HasTick && action.IsRunning)
    ...
    public void PlayClipTransition(ActiveAbility ability, ClipTransition customAnim = null)
    {
        if (ability.Definition.AnimationClip || ability.Definition.UseCustomAnim)
        {
            ...
            if (ability.Definition.IsBasicAttack)
            {
                ...
                animSpeed = Mathf.Clamp(animSpeed, ability.Definition.AnimSpeedCapRange.x, ability.Definition.AnimSpeedCapRange.y);

                
                Debug.Log($"$Attack speed {attackSpeedStat} Calculated anim speed : " +  animSpeed);
            }
            else
            {
